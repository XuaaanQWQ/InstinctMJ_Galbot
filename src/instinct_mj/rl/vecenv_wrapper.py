...
class InstinctRlVecEnvWrapper(VecEnv):
    ...
    def _pack_observations(self, obs_dict: dict) -> dict[str, torch.Tensor]:
        ...
        for exposed_name, source_group in self._group_map.items():
            packed[exposed_name] = self._flatten_group(group_name=source_group, group_obs=obs_dict[source_group])
    ...
    def _flatten_group(self, *, group_name: str, group_obs: torch.Tensor | dict[str, torch.Tensor]) -> torch.Tensor:
